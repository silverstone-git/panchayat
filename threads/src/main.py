...
from src.api.v1 import ideas, feed
...
async def handle_vote_event(payload):
    ...
    # The payload is wrapped in 'data' by the voting service
    ...
    if event_type == "VOTE_CAST":
        ...
        if idea_id and new_count is not None:
            logger.info(f"Updating idea {idea_id} vote_count to {new_count}")
            async with async_session() as db:
                await idea_service.update_vote_count(db, idea_id, new_count)
        else:
